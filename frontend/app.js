...
function showPanel(panelName, options = {}) {
  ...
  const shouldAnimate = animate && previousPanel && previousPanel !== requestedPanel;
  ...
  const applyPanelState = () => {
    ...
    document.querySelectorAll(".dashboard-panel").forEach((panel) => {
      ...
      if (isActive && !supportsViewTransition()) {
...
function fillDefaultDates() {
  ...
  setPickerInputValue("food-hour", currentHour, "food-hour-display", () => currentHour);
  setPickerInputValue("food-minute", currentMinute, "food-minute-display", () => currentMinute);
  setPickerInputValue("weight-hour", currentHour, "weight-hour-display", () => currentHour);
  setPickerInputValue("weight-minute", currentMinute, "weight-minute-display", () => currentMinute);
  if (!$("workout-hour").value) {
    setPickerInputValue("workout-hour", currentHour, "workout-hour-display", () => currentHour);
  }
  if (!$("workout-minute").value) {
    setPickerInputValue("workout-minute", currentMinute, "workout-minute-display", () => currentMinute);
  }
  ...
  $("toggle-register-pantry-btn")?.addEventListener("click", () => {
    ...
    if (isHidden) {
      requestAnimationFrame(() => {
        ...
        (submitButton || panel).scrollIntoView({ behavior: "smooth", block: "end" });
      });
    }
  });
